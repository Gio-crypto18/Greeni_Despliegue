...
public class MedicionController {
    ...
    @PostMapping
    public void insertar(@RequestBody MedicionfullDTO sdto) {
    ...
    @GetMapping
    public List<MedicionfullDTO> listar() {
        return service.list().stream().map(y -> {
            ModelMapper m = new ModelMapper();
            return m.map(y, MedicionfullDTO.class);
    ...
    @PreAuthorize("hasAuthority('ADMIN') or hasAuthority('CIENTIFICO')")
    @PutMapping //("/{id}")
    public ResponseEntity<String> modificar(@RequestBody MedicionfullDTO dto) {
        ModelMapper m = new ModelMapper();
    ...
    public ResponseEntity<?> listarId(@PathVariable("id") Integer id) {
        Medicion dev = service.listId(id);
        if (dev == null) {
        ...
        ModelMapper m = new ModelMapper();
        MedicionfullDTO dto = m.map(dev, MedicionfullDTO.class);
