...
public class GuiaController {
    ...
    @GetMapping("/{id}")
    public ResponseEntity<GuiaDTO> listarId(@PathVariable("id") Integer id) {
        Guia guia = guiaService.listId(id);
        if (guia == null) {
            return new ResponseEntity<>(HttpStatus.NOT_FOUND);
        }
        ModelMapper m = new ModelMapper();
        GuiaDTO dto = m.map(guia, GuiaDTO.class);
