...
public class PlantaController {
    ...
    @GetMapping
    public List<PLANTADTOFULLDTO> listar(){
        return ps.list().stream().map(y->{
            ModelMapper m = new ModelMapper();
            return m.map(y,PLANTADTOFULLDTO.class);
        }).collect(Collectors.toList());
    }
    @PreAuthorize("hasAuthority('ADMIN') or hasAuthority('CIENTIFICO')")
    @PostMapping
    public void insertar(@RequestBody PLANTADTOFULLDTO dto)
    {
        ModelMapper m = new ModelMapper();
    ...
    public ResponseEntity<?> listarId(@PathVariable("id") Integer id) {
        ...
        ModelMapper m = new ModelMapper();
        PLANTADTOFULLDTO dto = m.map(dev, PLANTADTOFULLDTO.class);
