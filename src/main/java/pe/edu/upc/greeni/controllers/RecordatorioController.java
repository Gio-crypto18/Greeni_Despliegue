...
import pe.edu.upc.greeni.dtos.RecordatoriofullDTO;
...
public class RecordatorioController {
    ...
    @GetMapping
    public List<RecordatoriofullDTO> listar() {
        return rR.list().stream().map(y -> {
            ModelMapper m = new ModelMapper();
            return m.map(y, RecordatoriofullDTO.class);
        }).collect(Collectors.toList());
    }

    @PreAuthorize("hasAuthority('ADMIN') or hasAuthority('PLANTLOVER')")
    @PostMapping
    public void insertar(@RequestBody RecordatoriofullDTO dto)
    {
        ModelMapper m = new ModelMapper();
    ...
    public ResponseEntity<?> listarId(@PathVariable("id") Integer id) {
        ...
        ModelMapper m = new ModelMapper();
        RecordatoriofullDTO dto = m.map(dev, RecordatoriofullDTO.class);
    ...
    @PreAuthorize("hasAuthority('ADMIN') or hasAuthority('PLANTLOVER')")
    @PutMapping //("/{id}")
    public ResponseEntity<String> modificar( @RequestBody RecordatoriofullDTO dto) {
        ModelMapper m = new ModelMapper();
